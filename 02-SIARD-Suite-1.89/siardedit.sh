#!/bin/sh
#== siardedit.sh =======================================================
# siardedit executes bin/SiardEdit.jar. 
# Version    : $Id: siardedit.sh 1507 2013-02-19 07:29:07Z hartwig $
# Application: Siard2
# Platform   : LINUX/UNIX
#-----------------------------------------------------------------------
...
{
  echo "Calling syntax                                                      "
  echo "  siardedit.sh [-h] | [-32] [<siardfile>]                           "
...
#-----------------------------------------------------------------------
# executes bin/SiardEdit.jar
#-----------------------------------------------------------------------
execute()
{
  execdir="$0"
  execdir=${execdir%/siardedit.sh}
  echo "$java" $JAVA_OPTS -jar "$execdir/bin/SiardEdit.jar" "$args"
  if [ ${#args} -eq 0 ];
  then
    "$java" $JAVA_OPTS -jar "$execdir/bin/SiardEdit.jar"
  else
