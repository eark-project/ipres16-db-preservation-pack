#!/bin/sh
#==$Workfile: siardfromdb.sh $==========================================
# siardfromdb executes ch.admin.bar.siard.SiardFromDb in bin/SiardEdit.jar. 
# Version    : $Id: siardfromdb.sh 1507 2013-02-19 07:29:07Z hartwig $
# Application: Siard2
# Platform   : LINUX/UNIX
#-----------------------------------------------------------------------
...
#-----------------------------------------------------------------------
# executes ch.admin.bar.siard.SiardFromDb in bin/SiardEdit.jar
#-----------------------------------------------------------------------
execute()
{
  execdir="$0"
  execdir=${execdir%/siardfromdb.sh}
  echo "$java" -Xmx1024m $JAVA_OPTS -cp "$execdir/bin/SiardEdit.jar"  "ch.admin.bar.siard.SiardFromDb" "$args"
  if [ ${#args} -eq 0 ];
  then
    "$java" -Xmx1024m $JAVA_OPTS -cp "$execdir/bin/SiardEdit.jar"  "ch.admin.bar.siard.SiardFromDb"
  else
    "$java" -Xmx1024m $JAVA_OPTS -cp "$execdir/bin/SiardEdit.jar"  "ch.admin.bar.siard.SiardFromDb" "$args"
  fi
  return $?
} # execute
